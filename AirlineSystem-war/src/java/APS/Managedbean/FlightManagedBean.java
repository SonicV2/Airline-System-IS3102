...
//        userID = (String) session.getAttribute("isLogin");
    ...
    public void generateForecast(ActionEvent event) {
        ...
        if (year == 0) {
            ...
            message = new FacesMessage(FacesMessage.SEVERITY_INFO, "Please enter a year in the future after " + tmp.get(Calendar.YEAR) + "!", "");
            FacesContext.getCurrentInstance().addMessage(null, message);
            return;
        }
//        else if (!demandForecastSessionBean.isValidYear()) {
...
//        }
        ...
        //Set status to update if the forecast for the year already exists
        if (demandForecastSessionBean.getForecast(year, routeId) != null) {
            isUpdate = true;
        }
    ...
    //Returns true if the year is to far away from the year now
    public boolean isFarApart(int year) {
        TimeZone tz = TimeZone.getTimeZone("GMT+8:00"); //Set Timezone to Singapore
        Calendar tmp = Calendar.getInstance(tz); //Get the current time
